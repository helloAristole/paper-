...
def model_metrics(clf, X_train, X_test, y_train, y_test):
    ...
    label = ["Train - AUC:{:.2f}%".format(auc(fpr_train, tpr_train) * 100),
             "Test - AUC:{:.2f}%".format(auc(fpr_test, tpr_test) * 100)]
...
svm_rbf =  svm.SVC(C=17, gamma=0.039 , probability=True,kernel='rbf',class_weight='balanced')
svm_rbf.fit(X_train, y_train)
model_metrics(svm_rbf, X_train, X_test, y_train, y_test)
scores = cross_val_score(svm_rbf, X, Y, cv=5)  # 计算交叉验证的得分
print(scores.mean())
